...
export function MapGrid({ properties, currentUser }: MapGridProps) {
      ...
      gridCols.push(
        <Tooltip key={cellKey}>
          <TooltipTrigger asChild>
            <div
              className={cn(
                'flex h-8 w-8 items-center justify-center border border-dashed border-white/10 transition-colors',
            ...
            >
              {property && <Home className="h-4 w-4 text-primary-foreground" />}
            </div>
          </TooltipTrigger>
          <TooltipContent>
            <div className="flex flex-col gap-1 p-1">
              <p className="font-bold">Coordenadas: [{x}:{y}:{property?.coord_z}]</p>
  ...
  return (
    <TooltipProvider>
      <div className="bg-background p-2 border rounded-md inline-block">
        {gridRows}
      </div>
    </TooltipProvider>
  );
